...
namespace OSCAL_SAR_Mapper
{
    public partial class _Default : Page
    {

         public const string XMLNamespace = @"http://csrc.nist.gov/ns/oscal/1.0";
        public const string SARschema = "SAR_XML_Template.xml";
        ...
        public string TemplateFile = "FedRAMP-SAR-Mapped.docx";
       ...
       // CurrentTask longRunningClass = new CurrentTask();
        ...
        {
            //string TemplateFile = "FedRAMP-SSP-Moderate-Baseline-Template.docx";

            var convertPageTop = HttpContext.Current.Server.MapPath(string.Format(@"~\Template\{0}", "Processing2.html"));
            ...
            if (FileUpload1.HasFile)
            {
                try
                {
                  ...
                  //  PrintProgressBar("", 0);
                    Filename = filename;
                    message = string.Format("Starting the conversion to Word of the OSCAL SAR {0}", filename);
                    percent = 2;
                    ...
                    else
                    {
                      ...
                      /*  myXMLElement = GetXMLElement(filename, "security-sensitivity-level");
                        ...
                        */
                        ...
                        string xmlSchemaPath = HttpContext.Current.Server.MapPath(string.Format(@"~\Template\{0}", SARschema));
        ...
        public static void ProcessData(string ssp_file, string word_template_file, out string wordDocumentPath)
        {
            ...
            string templateDocument = HttpContext.Current.Server.MapPath(string.Format(@"~/Template/{0}", word_template_file));
        ...
        public void FillPropertyCheckBoxes(List<Word.ContentControl> BaselineCheckControls, string xmlFile, string xmlSchema, string XMLNamespace, string securityIdPropFile)
        {

            string securityIdPropFilePath = HttpContext.Current.Server.MapPath(string.Format(@"~\Template\{0}", securityIdPropFile));
            ...
            SecurityControls = GetSecurityControls(xmlFile, xmlSchema, XMLNamespace);
        ...
        protected void ProcessWordDocument(string wordDocumentPath, string uploadedFilename, string BaselinePropCountFile)
        {
            ...
            string xmlSchemaPath = HttpContext.Current.Server.MapPath(string.Format(@"~\Template\{0}", SARschema));
            FillPropertyCheckBoxes(checkControls, xmlFilePath, xmlSchemaPath, XMLNamespace, BaselinePropCountFile);
